...
def plot_heat_map(resized_gcam,tr_label,pr_label,save_path):
    action_name=['drink water','eat meal/snack',
    'brushing teeth',
    'brushing hair',
    'drop',
    'pickup',
    ...
    l1=action_name[tr_label[0]-1]
    l2=action_name[pr_label[0]-1]
...
def plot_heat_map_multilayer(feature,save_path,layer):
    ...
    plt.savefig(save_path+'layer='+str(layer))
    # plt.savefig(save_path)
    plt.clf()
    plt.close('all')
...
def confusion_mat(mat,save_path):
    ...
    ax.set_xticklabels(action_name, rotation=0,fontsize=5)
